...
\begin{cards}{F1} %yhdistä kaavan nimi tai merkintä kaavaan
\pair{paino}{$\vec{G}=m\vec{g}$}
\pair{Newton II}{$\sum \vec{F}=m\vec{a}$}
\pair{jousen potentiaalienergia}{$\Delta E_\text{p}=\dfrac{1}{2}k(\Delta x)^2$}
\pair{harmoninen voima}{$\vec{F}=-k\Delta \vec{x}$}
\pair{Newton III}{$ \vec{F}_{12}=-\vec{F}_{21} $}
...
\pair{energian säilymislaki}{$ \dfrac{\d E}{\d t}=0 $}
\pair{Newton I}{$ \sum \vec{F}=\vec{0} \Rightarrow (\vec{a}=\vec{0} \Leftrightarrow \vec{v}=\text{vakio})$}
\pair{Newtonin painovoimalaki}{$G=\gamma\dfrac{m_1m_2}{r^2}$}
\pair{noste}{$ N=\rho V g $}
\pair{normaali-ilmanpaine}{$ p_0 $}
\pair{hydrostaattinen paine}{$p=\rho g h$}
\pair{voiman tekemä työ}{$ W=\int\vec{F}\cdot \d\vec{s} $}
\pair{teho}{$P=\dfrac{\d E}{\d t}$}
\pair{liukukitka}{$ F_\upmu=\mu N $}
\pair{paine}{$ P=\dfrac{F}{A} $}
\pair{lepokitka}{$ F_{\upmu_0}=\mu_0 N $}
\pair{keskinopeus}{$ \vec{v}_\text{k}=\dfrac{\Delta \vec{x}}{\Delta t} $}
\pair{keskikiihtyvyys}{$ \vec{a}_\text{k}=\dfrac{\Delta \vec{v}}{\Delta t} $}
\pair{(hetkellinen) nopeus}{$ \vec{v}=\dfrac{\d \vec{x}}{\d t} $}
\pair{(hetkellinen) kiihtyvyys}{$ \vec{a}=\dfrac{\d \vec{v}}{\d t} $}
\pair{liikemäärän säilymislaki}{$ \sum \vec{p}_\text{alussa} =\sum \vec{p}_\text{lopussa}$}
\pair{impulssi}{$\vec{I}=\vec{F}\Delta t=\Delta \vec{p}$}
\pair{Coulombin laki}{$ F=\dfrac{q_1q_2}{4\pi\epsilon_\text{r}\epsilon_0r^2} $}
\pair{sijainti tasaisessa kiihdytyksessä}{$ \vec{x}=\vec{x}_0+\vec{v}_0t+\dfrac{1}{2}\vec{a}(\Delta t)^2 $}
%\card{mo}
\end{cards}
...
\pair{reaktio rakenteilla}{reaktionimi}
